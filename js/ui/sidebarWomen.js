import { productsPage, getProducts } from "../shopWomen.js";
...
document.getElementById("sort-by-price-low-to-high").addEventListener("click", (e) => {
  e.preventDefault();
  sortFn = (a, b) => parseFloat(a.prices.price) - parseFloat(b.prices.price);
  sortAndFilterProducts();
});

document.getElementById("sort-by-price-high-to-low").addEventListener("click", (e) => {
  e.preventDefault();
  sortFn = (a, b) => parseFloat(b.prices.price) - parseFloat(a.prices.price);
  sortAndFilterProducts();
});

document.getElementById("sort-by-name-a-to-z").addEventListener("click", (e) => {
  e.preventDefault();
  sortFn = (a, b) => a.name.localeCompare(b.name);
  sortAndFilterProducts();
});

document.getElementById("sort-by-name-z-to-a").addEventListener("click", (e) => {
  e.preventDefault();
...
const sortAndFilterProducts = async () => {
  ...
  if (selectedColor !== "") {
    filteredProducts = filteredProducts.filter(product => {
      return product.tags.some(tag => tag.name.toLowerCase() === selectedColor.toLowerCase());
    });
  }
  ...
  if (selectedPrice !== "") {
    ...
    filteredProducts = filteredProducts.filter(product => {
      return parseInt(product.prices.price) >= parseInt(minPrice) &&
